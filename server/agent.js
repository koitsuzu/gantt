...
所有關於「今天」「明天」「下週」「本月」的推算，都**必須**以上述日期為絕對基準進行推演。嚴禁自行假設年份為 2000 年或 1970 年。

### 💡 資訊不足時的「溫柔引導與猜測」
當使用者輸入的指令過於簡略或資訊不足以呼叫特定工具時：
1. **絕對禁止**直接報錯或回覆「無法執行」。
2. **溫柔詢問**：請先以親切、如同真人助理般的口吻，主動猜測使用者的意圖並向其確認。
   - *範例*：「您剛剛提到的『刪除』，是指要刪除昨天新增的『測試任務』嗎？還是其他專案呢？😊」
3. **提供快捷選項**：在你的溫柔回覆最後，**必須**換行並使用 \`[選項] 建議的完整指令\` 的格式，列出 1~3 個最可能的意圖，讓前端能將其轉化為按鈕供使用者點擊。
   - *範例*：
     \`[選項] 查詢目前所有風險專案\`
     \`[選項] 查詢 A 專案的詳細進度\`
     \`[選項] 查看各部門的工作負載\`
...
async function handleGroqChat(db, sessionId, userMessage, sharedContext = '') {
    ...
    while (isToolCalling) {
        ...
        try {
            ...
            console.error('🔍 [Consultant/Groq] API Error:', err);
            return { reply: '😅 不好意思，我在查詢資料時遇到了一些技術問題。能麻煩您再描述一次想了解的內容嗎？我會盡力幫您找到答案！😊', tools_called: toolsCalled };
        }
        ...
        if (!toolCalls || toolCalls.length === 0) {
            ...
            for (const toolCall of toolCalls) {
                ...
                try {
                    ...
                    console.error(`🔧 [Groq] Failed to parse args for ${name}:`, toolCall.function.arguments);
                    args = {};
                }
...
async function handleChat(db, sessionId, userMessage, sharedContext = '') {
    if (provider === 'gemini') {
        return await handleGeminiChat(db, sessionId, userMessage, sharedContext);
    } else if (provider === 'groq') {
        return await handleGroqChat(db, sessionId, userMessage, sharedContext);
    }
